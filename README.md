...
| `tools/desktop-preview/` | کامپایل و عکس گرفتن از صفحه‌ها بدون SDK اندروید، و تست کامل اپ با سرور |
| `design/` | فایل‌های طراحی Claude Design |
| `scripts/setup.sh` | نصب ابزارهای لازم برای ساخت |
| `AGENTS.md` | راهنمای کامل برای Codex و ایجنت‌های دیگر |

## ادامه‌ی کار با Codex یا ابزارهای دیگر

- **راهنمای کامل برای ایجنت‌ها:** فایل [`AGENTS.md`](AGENTS.md) (انگلیسی) همه‌چیز رو توضیح می‌ده: تصمیم‌های محصول، ساختار کد، دستورهای ساخت و تست، API سرور، نکته‌های راست‌به‌چپ و فهرست کارهای بعدی. Codex این فایل رو خودکار می‌خونه.
- **آماده‌سازی محیط:** `./scripts/setup.sh` یک بار اجرا بشه؛ SDK اندروید و بسته‌های سرور رو نصب می‌کنه.
- **طراحی اصلی:** همه‌ی فایل‌های خروجی Claude Design در پوشه‌ی `design/` هست (عکس صفحه‌ها در `design/shots/`).
- **پیش‌نمایش بدون SDK اندروید:** `tools/desktop-preview` کد مشترک اپ رو روی کامپیوتر اجرا می‌کنه و از صفحه‌ها عکس می‌گیره.
